  ...
  if (payload.products) {
    ...
    for (const p of payload.products) {
      try {
        ...
        console.warn(`[Sync] Error procesando producto ${p.code}:`, error);
        errorCount++;
      }
    }
    
    console.log(`[Sync] Productos procesados: ${successCount} exitosos, ${errorCount} errores`);
  }
  ...
  if (payload.stockMoves) {
    ...
    for (const mv of payload.stockMoves) {
      ...
      try {
        ...
        console.warn(`[Sync] Error procesando movimiento ${mv.id} para producto ${mv.productCode}:`, moveError);
        // Continuamos con el siguiente movimiento
      }
    }
